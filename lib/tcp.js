...
function handleSocket(socket) {
	...
	var chuck_handle = {
		get_content_length: function(chunk) {
			...
			chuck_handle.run(_remain_buffer)
		},
		...
		parse_content: function() {
			...
			try {
				...
				var result = JSON.parse(_result_str);
				_buffer_cache = [];
			} catch (e) {
				console.error("数据解析出错", e);
				_buffer_cache = [];
				return;
			}
	...
	socket.on("msg", function(data) {
		if (data.type && data.info) {
			$$.runGEN(function*(resume) {
				var _group = console.flagHead("<<< msg:" + data.type + (data.from ? (":" + data.from) : "")) + " " + (data.msg || "");
	...
	socket.on("error", function(e) {
		console.flag("TCP ERROR", e);
	});
	socket.on("close", function(e) {
		console.flag("TCP CLOSE", e);
	});
	return socket;
};
